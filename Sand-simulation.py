...
def pr():
    ...
    for Px,Py in Actu:
        py.draw.rect(screen,MAPVAL[Py*SizeX+Px][0],(10+Px*TPix,10+Py*TPix,TPix,TPix))
    ...
    screen.blit(font.render("Rendu : "+str(round((time.time()-at)*1000))+"ms",True,(255,255,255)),(SizeX*TPix+30,65))
    screen.blit(font.render("Simu : "+str(Rt)+"ms",True,(255,255,255)),(SizeX*TPix+30,85))
    py.display.flip()
def simulate():
    ...
    for i,Pos in enumerate(Actifs):
        New=Pos
        Ax,Ay=Pos
        Mypoid=Poids[MAP[Ay*SizeX+Ax]]
        Mygravit=Gravity[MAP[Ay*SizeX+Ax]]
        ...
        if New[0]==Ax and MAP[New[1]*SizeX+New[0]]==2:
            ...
            if 0<=New[0]+w<=SizeX-1:
                if Poids[MAP[Ay*SizeX+Ax+w]]<Mypoid:
                    New=(Ax+w,Ay)
        #New correspond à la nouvelle position
        if Pos!=New:
            #échanger les particules
            MAP[Ay*SizeX+Ax],MAP[New[1]*SizeX+New[0]]=MAP[New[1]*SizeX+New[0]],MAP[Ay*SizeX+Ax]
            MAPVAL[Ay*SizeX+Ax],MAPVAL[New[1]*SizeX+New[0]]=MAPVAL[New[1]*SizeX+New[0]],MAPVAL[Ay*SizeX+Ax]
            #changer, au besoin, les coordonnées de actifs
            if MAPVAL[Ay*SizeX+Ax][1]==0:
                Actifs[i]=New
            #actualiser l'écran
            Actu.append(New)
            Actu.append((Ax,Ay))
            ...
            if Ay!=SizeY-1-limY:
                if Move[MAP[(Ay-Mygravit)*SizeX+Ax]]:
                    if MAPVAL[(Ay-Mygravit)*SizeX+Ax][1]==0:Actifs.append((Ax,Ay-Mygravit));MAPVAL[(Ay-Mygravit)*SizeX+Ax][1]=1
                if Ax>0:
                    if Move[MAP[(Ay-Mygravit)*SizeX+Ax-1]]:
                        if MAPVAL[(Ay-Mygravit)*SizeX+Ax-1][1]==0:Actifs.append((Ax-1,Ay-Mygravit));MAPVAL[(Ay-Mygravit)*SizeX+Ax-1][1]=1
                if Ax<SizeX-1:
                    if Move[MAP[(Ay-Mygravit)*SizeX+Ax+1]]:
                        if MAPVAL[(Ay-Mygravit)*SizeX+Ax+1][1]==0:Actifs.append((Ax+1,Ay-Mygravit));MAPVAL[(Ay-Mygravit)*SizeX+Ax+1][1]=1
        elif Act[MAP[Ay*SizeX+Ax]]==0:
            #si ne bouge pas et est désacitvable
            if MAPVAL[Ay*SizeX+Ax][1]==1:
                MAPVAL[Ay*SizeX+Ax][1]=0
                #désactiver la particule
            Actu.append((Ax,Ay))
    Actifs=[(Ax,Ay) for Ax,Ay in Actifs if MAPVAL[Ay*SizeX+Ax][1] or Act[MAP[Ay*SizeX+Ax]]]
    return Actu
RUN=0;Rt=0
while running:
    if RUN:
        ...
        if 0<xs<10+SizeX*TPix and 0<ys<10+SizeY*TPix:
            ...
            for dx in range(Tspawn):
                for dy in range(Tspawn):
                    ...
                    if yspawn+dy!=0:
                        if Move[MAP[(yspawn+dy-1)*SizeX+xspawn+dx]]:
                            if MAPVAL[(yspawn+dy-1)*SizeX+xspawn+dx][1]==0:Actifs.append((xspawn+dx,yspawn+dy-1));MAPVAL[(yspawn+dy-1)*SizeX+xspawn+dx][1]=1
                        if xspawn+dx>0:
                            if Move[MAP[(yspawn+dy-1)*SizeX+xspawn+dx-1]]:
                                if MAPVAL[(yspawn+dy-1)*SizeX+xspawn+dx-1][1]==0:Actifs.append((xspawn+dx-1,yspawn+dy-1));MAPVAL[(yspawn+dy-1)*SizeX+xspawn+dx-1][1]=1
                        if xspawn+dx<SizeX-1:
                            if Move[MAP[(yspawn+dy-1)*SizeX+xspawn+dx+1]]:
                                if MAPVAL[(yspawn+dy-1)*SizeX+xspawn+dx+1][1]==0 and Move[MAP[(yspawn+dy-1)*SizeX+xspawn+dx+1]]==1:Actifs.append((xspawn+dx+1,yspawn+dy-1));MAPVAL[(yspawn+dy-1)*SizeX+xspawn+dx+1][1]=1
                    #ajoute les particules si elles sont différentes de celles de la grille, car sinon rend un grésillement
                    if MAP[(yspawn+dy)*SizeX+dx+xspawn]!=SpawnT:
                        MAP[(yspawn+dy)*SizeX+dx+xspawn]=SpawnT
                        if Move[SpawnT]:
                            #si l'ancienne particule était active, il ne faut pas ajouter ses coordonnées une deuxième fois dans Actifs
                            if MAPVAL[(yspawn+dy)*SizeX+xspawn+dx][1]==0:
                            ...
                            MAPVAL[(yspawn+dy)*SizeX+dx+xspawn]=[C,Move[SpawnT],(0,0)]
                        else:
                            C=col[SpawnT]
                            MAPVAL[(yspawn+dy)*SizeX+dx+xspawn]=[C,Move[SpawnT],(0,0)]
